 ...
 *
 * @uses mega_charity_header_style()
 */
function mega_charity_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'mega_charity_custom_header_args', array(
		'default-image'          => '%s/assets/uploads/header-image.jpg',
		'default-text-color'     => '000',
		'width'                  => 1200,
		'height'                 => 528,
		'flex-height'            => true,
		'wp-head-callback'       => 'mega_charity_header_style',
	) ) );
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'mega-charity' ),
		),
	) );
}
add_action( 'after_setup_theme', 'mega_charity_custom_header_setup' );

if ( ! function_exists( 'mega_charity_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see mega_charity_custom_header_setup().
	 */
	function mega_charity_header_style() {
		...
		wp_add_inline_style( 'mega-charity-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'mega_charity_header_style', 10 );